...
def _load_binarization_config() -> dict[str, Any]:
    """Safely extract dynamic parameters with secure fallbacks"""
    try:
        with open(CONFIG_PATH, "r") as f:
            config = yaml.safe_load(f)
            img_proc = config['pipeline_settings']['image_processing']
            bin_settings = config['pipeline_settings']['binarization']
            return {
                "min_prob_threshold": img_proc['min_prob_threshold'],
                "min_trace_size": bin_settings['min_trace_size'],
                "min_background_size": bin_settings['min_background_size']
            }
    except (FileNotFoundError, KeyError) as e:
        print(f"[WARNING]: Failed to instantiate from config file: {e}")
        print("Falling back to fail-safe defaults")
        return {"min_prob_threshold": 0.95, "min_trace_size": 6, "min_background_size": 4}
...
def binary_image(
    ...
    cfg = _load_binarization_config()
    if min_trace_size is None:
        min_trace_size = cfg["min_trace_size"]
    if min_background_size is None:
        min_background_size = cfg["min_background_size"]

    if image.ndim != 2:
        image = color.rgb2gray(image)
    if markers_background is None:
        markers_background = get_background_markers(image, cfg["min_prob_threshold"])
    ...
    image_bin = remove_small_segments_and_edges(
...
def remove_small_segments_and_edges(
    ...
    remove_small_objects(
        image_bin, min_size=min_trace_size, connectivity=2, in_place=True
    )
    image_bin = ~image_bin
    remove_small_objects(
        image_bin, min_size=min_edge_length, connectivity=2, in_place=True
    )
    image_bin = ~image_bin
    return image_bin
...
def fill_corners(image_bin: npt.NDArray[Any]) -> npt.NDArray[np.bool_]:
    ...
    corners = (
        (upper_left == 3) | (upper_right == 3) | (lower_left == 3) | (lower_right == 3)
    )
...
def peak_local_max_rows(
    ...
    maxima = np.logical_and(
        np.hstack((np.ones((a.shape[0], 1)) * include_border, a[:, 1:] >= a[:, :-1])),
        np.hstack((a[:, :-1] >= a[:, 1:], np.ones((a.shape[0], 1)) * include_border)),
    )
    return maxima
...
def peak_local_max_cols(
    ...
    maxima = np.zeros_like(a, dtype=bool)
    maxima = np.logical_and(
        np.vstack((np.ones((1, a.shape[1])) * include_border, a[1:, :] >= a[:-1, :])),
        np.vstack((a[:-1, :] >= a[1:, :], np.ones((1, a.shape[1])) * include_border)),
    )
    return maxima
