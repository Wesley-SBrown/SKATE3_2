...
def get_segments(
    ...
    print(f"found {num_traces} segments")
    Record.record("num_segments", num_traces)

    segments = img_seg_to_seg_objects(
        image_segments, num_segments, ridges_h, ridges_v, img_gray
    )
    ...
    if Record.active:
        ...
        centerlines = [
            seg.center_line for seg in segments.values() if seg.has_center_line
        ]
...
def ridges_to_centerline(ridge_h_coords, ridge_v_coords):
    ...
    # print ""
    # print(f"ridge_v_coords {ridge_v_coords}")
    # print(f"ridge_v_cols {ridge_v_cols}")
    # print(f"ridge_h_cols {ridge_h_cols}")

    # If ridges vertical at ends, truncate
    if truncate:
        # print("TRUNCATING RIDGE!")
        # print(f"domain_h {domain_h}")
        # print(f"domain_v {domain_v}")
        if domain_v[0] <= domain_h[0]:
            ridge_v_coords = ridge_v_coords[(ridge_v_coords[:, 1] != domain_v[0]), :]
        if domain_v[1] >= domain_h[1]:
            ridge_v_coords = ridge_v_coords[(ridge_v_coords[:, 1] != domain_v[1]), :]
        # print(f"trunacted ridge_v_coords {ridge_v_coords}")
